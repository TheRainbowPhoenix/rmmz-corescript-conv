...
function renderControlSwitch(p) {
  ...
  const target =
    Number(start) === Number(end)
      ? `Switch[${Number(start)}]`
      : `Switch[${Number(start)}:${Number(end)}]`;
...
function renderControlVariable(p) {
  ...
  else if (Number(operandType) === 1) rhs = `Variables[${Number(p?.[4] ?? 0)}]`;
...
function decodeIfCondition(params) {
  ...
  if (t === 1) {
    ...
    const rhs =
      rhsType === 0
        ? String(params[3] ?? 0)
        : `Variables[${Number(params[3] ?? 0)}]`;
    return `Variables[${id}] ${op} ${rhs}${variableComment(id)}`;
  }
  if (t === 2) {
  ...
  if (t === 4) {
    ...
    if (mode === 0) return `Actors[${actorId}].inParty == True`;
    if (mode === 1)
      return `Actors[${actorId}].name == ${JSON.stringify(String(params[3] ?? ""))}`;
    if (mode === 2)
      return `Actors[${actorId}].classId == ${Number(params[3] ?? 0)}`;
    if (mode === 3)
      return `Actors[${actorId}].skillId == ${Number(params[3] ?? 0)}`;
    if (mode === 4)
      return `Actors[${actorId}].weaponId == ${Number(params[3] ?? 0)}`;
    if (mode === 5)
      return `Actors[${actorId}].armorId == ${Number(params[3] ?? 0)}`;
    if (mode === 6)
      return `Actors[${actorId}].stateId == ${Number(params[3] ?? 0)}`;
  }
  if (t === 7)
    return `Gold ${["<=", ">=", "<", ">", "=="][Number(params[2] ?? 0)] ?? ">="} ${Number(params[1] ?? 0)}`;
  if (t === 8) return `Item[${Number(params[1] ?? 0)}] == owned`;
  if (t === 9)
    return `partyHas(Weapons[${Number(params[1] ?? 0)}], includeEquipped=${Number(params[2] ?? 0) ? "True" : "False"})`;
  if (t === 10)
    return `partyHas(Armors[${Number(params[1] ?? 0)}], includeEquipped=${Number(params[2] ?? 0) ? "True" : "False"})`;
  if (t === 11)
...
function toHtml(lines) {
  ...
  const colorize = (s) => {
    ...
    out = out.replace(/Variables\[(\d+)\]/g, (_, id) => {
      const name = String(variableNames?.[Number(id)] ?? "").trim();
      if (!name) return `Variables[${id}]`;
      return `Variables[${id}<span class="hint-badge" style="margin-left:4px;padding:0 4px;border-radius:10px;background:#2d333b;color:#9cdcfe;border:1px solid #4b5563;font-size:10px;user-select:none" title="${esc(name)}">${esc(name)}</span>]`;
    });
    out = out.replace(/Variable\[(\d+)\]/g, (_, id) => {
      const name = String(variableNames?.[Number(id)] ?? "").trim();
      if (!name) return `Variables[${id}]`;
      return `Variables[${id}<span class="hint-badge" style="margin-left:4px;padding:0 4px;border-radius:10px;background:#2d333b;color:#9cdcfe;border:1px solid #4b5563;font-size:10px;user-select:none" title="${esc(name)}">${esc(name)}</span>]`;
    });
    out = out.replace(/Switch\[(\d+)\]/g, (_, id) => {
      const name = String(switchNames?.[Number(id)] ?? "").trim();
      if (!name) return `Switch[${id}]`;
      return `Switch[${id}<span class="hint-badge" style="margin-left:4px;padding:0 4px;border-radius:10px;background:#2d333b;color:#9cdcfe;border:1px solid #4b5563;font-size:10px;user-select:none" title="${esc(name)}">${esc(name)}</span>]`;
    });
    out = out.replace(/Switches\[(\d+)\]/g, (_, id) => {
      const name = String(switchNames?.[Number(id)] ?? "").trim();
      if (!name) return `Switch[${id}]`;
      return `Switch[${id}<span class="hint-badge" style="margin-left:4px;padding:0 4px;border-radius:10px;background:#2d333b;color:#9cdcfe;border:1px solid #4b5563;font-size:10px;user-select:none" title="${esc(name)}">${esc(name)}</span>]`;
    });
    out = out.replace(
      /\b(true|false|True|False|ON|OFF)\b/g,
      '<span style="color:#dcdcaa">$1</span>',
    );
    out = out.replace(
      /\b(if|else|elif|while|goto|label)\b/g,
    ...
    out = out.replace(
      /\b(Variables|Switch|Weapons|Armors|Items|Actors|Enemies|Characters|Party|Other)\b/g,
  ...
  const body = lines
    .map((line) => {
      ...
      if (rest.startsWith("label ")) {
        ...
        return `${"&nbsp;&nbsp;&nbsp;&nbsp;".repeat(tabs)}<a id="label-${encodeURIComponent(label)}"></a><span style="color:#c586c0">${colorize(rest)}</span>`;
      }
      if (rest.startsWith("goto(")) {
        const m = rest.match(/^goto\(\"(.*)\"\)$/);
        const label = m ? m[1] : "";
        return `${"&nbsp;&nbsp;&nbsp;&nbsp;".repeat(tabs)}<a href="#label-${encodeURIComponent(label)}" style="color:#c586c0">${colorize(rest)}</a>`;
      }
      if (rest.trim().startsWith("#")) {
        return `${"&nbsp;&nbsp;&nbsp;&nbsp;".repeat(tabs)}<span style="color:#6a9955">${esc(rest)}</span>`;
      }
      if (!rest.startsWith("- ["))
        return `${"&nbsp;&nbsp;&nbsp;&nbsp;".repeat(tabs)}<span style="color:#d4d4d4">${colorize(rest)}</span>`;
      return `${"&nbsp;&nbsp;&nbsp;&nbsp;".repeat(tabs)}<span style="color:#d4d4d4">${colorize(rest.slice(2))}</span>`;
    })
    .join("\n");
  return `<pre style="background:#1e1e1e;color:#d4d4d4;padding:12px;border-radius:8px;line-height:1.45">${body}</pre>`;
}
